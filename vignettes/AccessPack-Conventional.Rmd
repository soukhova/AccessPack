...
- Population centers (triangles) near the left edge of the map (only in proximity to the small employment center) have the least amount of job accessibility (i.e. Population centers  `r toy_sim_zones_access %>% slice_min(c_access_jobs, n=1) %>% pull(id_short)` has the minimum job accessibility `r toy_sim_zones_access %>% slice_min(c_access_jobs, n=1, with_ties = F) %>% pull(c_access_jobs)`);

**What are the issues?**

What does a maximum of `r  max(toy_sim_zones_access$c_access_jobs, na.rm=T)` and a minimum of `r  min(toy_sim_zones_access$c_access_jobs, na.rm=T)` job accessibility mean exactly? We can normalize this value by considering population centers, employment centres, or the average global population or jobs. However, the value is not sensitive to the population at the origin seeking the opportunity (in this case jobs). For this reason, the value of $A_i$ is difficult to interpret. 
